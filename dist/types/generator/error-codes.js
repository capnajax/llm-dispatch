...
async function generate() {
    ...
    lines.push('};', '');
    lines.push('export const defaultStatusCodes:Partial<Record<ErrorCode, number>> = {');
    for (const m of runConfig.modules) {
        Object.entries(m.codes).forEach((k) => {
            const v = k[1];
    ...
    for (const m of runConfig.modules) {
        const typename = m.name.substring(0, 1).toUpperCase() + m.name.substring(1) + 'ErrorCode';
        if (m.description) {
            lines.push('/**');
            lines.push(...splitLines(m.description).map(l => ` * ${l}`));
            lines.push(' */');
