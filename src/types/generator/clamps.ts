...
function findTypeImport(
  ...
  if (localType) {
    ...
    return {
      typeName,
      moduleSpecifier: makeImportPath(
        outputPath,
  ...
  for (const declaration of sourceFile.getImportDeclarations()) {
    for (const namedImport of declaration.getNamedImports()) {
      ...
      throw new Error(
        `Could not resolve type ${typeName} imported ` +
  ...
  throw new Error(
    `Could not find a declaration or import for type ` +
...
async function generateFile(
  ...
  const typeImports = validators.map(
    validator =>
      findTypeImport(
        sourceFile,
        outputPath,
  ...
  for (const typeImport of typeImports) {
    let names = importsByModule.get(
      typeImport.moduleSpecifier
    );
  ...
  for (
    const [moduleSpecifier, typeNames]
    of importsByModule
  ) {
