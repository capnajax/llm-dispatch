...
interface GenerateResult {
  ...
  typeguards: Record<string, string>
}

function generate(specs: Record<string, PropertySpecType>): GenerateResult {
  const log = getLogger(MODULE, generate);
  const result = {types: {}, typeguards: {}} as GenerateResult;

  const parenAndIndent = (line:string, indent:number=0):string => {
  ...
  const generateTest =
    (name: string, spec: RefType|PropertySpecType, level?:number)
    : string[]|{isAtomic: boolean, lines: string[]} => {
      ...
      const trimFirstLine =
        (lines:string[]|{isAtomic: boolean, lines:string[]}):string[] => {
          const result = [];
      ...
      case 'oneOf':
        log.debug(
          'ANYOF/ONEOF on spec ' +
          JSON.stringify((spec as OneOfType|AnyOfType).union)
        );
        // not really making the distinction between oneOf and anyOf here.
        if ((spec as AnyOfType).union.length > 0) {
          const union = (spec as AnyOfType).union;
          const itemsOred = union.map((u):string => {
      ...
      case 'array':
        ...
        const items = generateTest(
          itemName, (spec as ArrayType).items, level + 2
        );
        ...
        if (itemsAr.length === 0) {
          ...
          result.push(
            `${h}${name}.every((${itemName}:any) => ${itemsAr[0].trim()})`
          );
        } else {
          const item:string[] = [];
      ...
      case 'float':
        ...
        if ((spec as NumberType).minimum) {
          result.push(`${h}${name} >= ${(spec as NumberType).minimum}`)
        }
        if ((spec as NumberType).maximum) {
          result.push(`${h}${name} <= ${(spec as NumberType).maximum}`)
        }
        if ((spec as NumberType).format) {
          switch ((spec as NumberType).format) {
          case 'unixtime':
            // no validation
            break;
          default:
            log.warn(`Unknown integer format "${(spec as NumberType).format}"`);
          }
        }
      ...
      case 'object':
        log.debug(
          'OBJECT on spec ' +
          JSON.stringify((spec as ObjectType).object)
        );
        result.push(`${h}( typeof ${name} === 'object' )`);
        const so = (spec as ObjectType).object;
        // for the extended class
        const se = (spec as ObjectType).extends;
        if (se) {
          const gt = generateTest(
            name,
            specs[se],
            level
          );
        ...
        if (so?.properties) {
          for (const oKey of Object.keys(so.properties)){
            ...
            log.debug(
              `gt = ${JSON.stringify(gtLines)}`
            );
            propertiesTests.push(...gtLines);
          }
        }
        if ((spec as ObjectType).nullable !== true) {
      ...
      case 'ref':
        result.push(`${h}is${(spec as RefType).ref}(${name})`);
        isAtomic = true;
        break;

      case 'string':
        if ((spec as StringType).enum) {
          result.push(`${h}${JSON.stringify((spec as StringType).enum)
            }.includes(${name})`);
          isAtomic = true;
        } else {
      ...
      if ((spec as Record<string,unknown>).nullable) {
        const newResult = [
          `( ${name} === null || (`,
          result.map(r => `  ${r}`).join(` &&\n${h}`),
          '))'
