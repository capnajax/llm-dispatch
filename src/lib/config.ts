...
import { Normalized } from '../types/types-tools.js';
import { Config } from '../types/connectivity-types.js';
import { ConfigAsLoaded, normalizeConfig, validateConfigAsLoaded } from '../types/validators/raw-config.js';
...
async function loadConfig(hostname: string): Promise<Config> {
  ...
  let configParsed!:ConfigAsLoaded;
  try {
  ...
  const configNormalization:Normalized<Config> =
    normalizeConfig(configParsed, hostname, filename);
  ...
  if (configNormalization.valid) {
    ...
    console.error(
      'Config incomplete for hostname "%s"%s',
      hostname,
      configNormalization.errors ? ':' : ''
    );
