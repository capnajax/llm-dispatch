import { OneOrMany } from "../types/logger-types.js";
  ...
  if (thing) {
    if (type === 'array') {
      ...
      throw new Error(`"${name}" is not ${
        type === 'object' ? 'an' : 'a'} "${type}`);
    }
  }
}

export function assertRecordHasProperties(
  name: string,
...
export function assertSingleItemRecord(
  name: string,
  object: Record<string, unknown>,
  propertyName: string,
  validateType?: 'array'|'string'|'number'|'object'
): void {
  ...
  if (unexpectedMembers.length > 1) {
    throw new Error(
      `object "${name}" has unexpected members: ${unexpectedMembers}`
    );
  }
  if (!Object.hasOwn(object, propertyName)) {
    throw new Error(
      `object "${name}" does not contain property "${propertyName}"`
    );
  }
  if (validateType) {
    assertType(
      `object "${name}" property "${propertyName}"`,
      object[propertyName],
      validateType
    );
  }
...
export function deepClone<T>(
    obj: T|null,
    options?: { without?: OneOrMany<string>}
): T|null {
  ...
  if (Array.isArray(obj)) {
    return obj.map(o => deepClone(o, options)) as any;
  } else if (obj === null) {
    return null;
...
export function deepMerge<T>(
  ...
  let result = deepClone(complete) as T;
...
export function pick (
  o: Record<string, unknown>, keys:string[], include: boolean = true
 ...
 *   the width.
 */
export function reflowText(text: string, nCols: number): string {
  if (nCols < 2) {
...
export function ofType(
  v:unknown, t: ((v:unknown)=>boolean)|string|RegExp, nullable: boolean = false
)
: boolean {
  return (
 ...
 * @returns `true` if the type of `v` validates to `t`, `false` if not.
 */
export function ofTypeOrUndefined(
  v:unknown, t: ((v:unknown)=>boolean)|string|RegExp, nullable: boolean = true
)
: boolean {
