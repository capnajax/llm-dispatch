...
import { CreateChatCompletionRequest } from '../types/generated/openai-types.js';
import { OpenAIRoleTypes, openAIRoleTypes, PromptConfig, PromptConfigPrompt } from './prompts-types.js';
...
export async function loadPromptsConfig(): Promise<void> {
  ...
  async function doLoad(): Promise<void> {
    const configBuf = await fs.readFile(
      path.join(path.dirname(process.argv[1]), 'prompts', 'prompts.yaml')
    );
      ...
      for (const pk of Object.keys(configRaw.prompts)) {
        ...
        if (pv.message) {
          ...
          if (typeof pv.message === 'object') {
            let role:OpenAIRoleTypes = 'system';
